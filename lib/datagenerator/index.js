...
const ENUMERATION_TYPES = Object.seal({
...
const getEnumerationValues = (lookupName, { count = 1, enumerationType = ENUMERATION_TYPES.StringSingle } = {}) => {
  ...
  case ENUMERATION_TYPES.StringSingle:
    return faker.helpers.arrayElements(lookupNameMap?.[lookupName], count)?.[0];
  case ENUMERATION_TYPES.StringMulti:
    return faker.helpers.arrayElements(lookupNameMap?.[lookupName], count);
  default:
...
const getNumericValue = (resourceName, fieldName, { max, scale = 2, precision = 0 } = {}) => {
  ...
  if (!precision || precision === 0) {
    ...
    return `${getRandomInteger(scale)}.${getRandomInteger(10, precision) - 1}`;
  }
};
...
async function* rcfGeneratorIterator({
  ...
  const records = Object.seal({
    Property: [
      {
        ListingKey,
        StandardStatus: getEnumerationValues('StandardStatus'),
        InteriorFeatures: getEnumerationValues('InteriorFeatures', { count: 2 }),
...
const generateRcfData = async ({
  ...
  try {
    ...
    // resourceName, array of RCF values
    const results = {};

    for (const resource of resources) {
      for await (const { records } of rcfGeneratorIterator({
        modelName: resource,
        metadataReportJson,
        relatedRecordCounts
      })) {
        //Object.keys(records).forEach(async resourceName => {
        if (outputPath) {
          await mkdir(outputPath, { recursive: true });
          await writeFile(
            resolve(join(outputPath, `${resource}.${getFileSafeIso8601Timestamp()}.json`)),
            JSON.stringify({
              '@reso.context': `urn:reso:metadata:2.0:${resource.toLocaleLowerCase()}`,
              value: records?.[resource]
            })
          );
        } else {
          if (!results?.[resource]) {
          ...
          results[resource].push(...(records?.[resource] ?? []));
        }
      }
    }
