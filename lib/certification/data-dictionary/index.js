...
const runDDTests = async ({
  ...
  for await (const config of configs) {
    ...
    try {
      ...
      archiveEndorsement({ resultsPath: RESULTS_PATH_NAME, providerUoi, providerUsi, recipientUoi, endorsementName, version, outputPath });
      ...
      cleanUpCertificationFiles(outputPath);
      ...
      copyDataDictionaryTestResults(RECIPIENT_PATH, outputPath);
    }
  }
 ...
 */
const cleanUpCertificationFiles = outputPath => {
  ...
  Object.values(CERTIFICATION_FILES).forEach(fileName => {
    const sourcePath = join(outputPath || process.cwd(), fileName);
 ...
 */
const copyDataDictionaryTestResults = (recipientPath, outputPath) => {
