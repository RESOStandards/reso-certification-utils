 ...
 */
const executeCommanderMetadataTest = async ({ pathToWebApiCommander, version, pathToConfigFile, useSpinner = false } = {}) => {
  let error, spinnerInstance;
  ...
  try {
    ...
    if (useSpinner) {
      spinnerInstance = ora();
      spinnerInstance.start('Running Data Dictionary metadata tests...');
    }
    ...
    await new Promise((resolve, reject) => {
      ...
      commander.stdout.on('data', x => {
        ...
        if (message) {
          ...
          if (processed?.length) {
            if (useSpinner && spinnerInstance) {
              for (let i = 0; i < processed.length; i++) {
                spinnerInstance.text = `Running Data Dictionary metadata tests... \n${processed.slice(i, i + 3).join('\n')}`;
  ...
  if (error) {
    const message = 'Data Dictionary metadata testing failed!';
    if (useSpinner) {
      spinnerInstance.fail(message);
    }
...
const runDDTests = async ({
  ...
  const { LOG_INFO } = getLoggers(fromCli);

  LOG_INFO(`Using strict mode: ${strictMode}!`);

  const SPINNER_INSTANCE = ora();
  ...
  for await (const config of configs) {
    ...
    try {
      ...
      if (runAllTests) {
        ...
        try {
          SPINNER_INSTANCE.info('Starting replication steps...');
