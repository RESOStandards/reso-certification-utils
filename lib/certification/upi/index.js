...
const runUpiTests = async ({
  ...
  if (!(Object.values(resoCommonFormatJson) || (pathToResoCommonFormatJson && pathToResoCommonFormatJson?.length))) {
  ...
  try {
    const parsedJson =
      (Object.values(resoCommonFormatJson) ? resoCommonFormatJson : JSON.parse(await readFile(pathToResoCommonFormatJson))) ?? {};
    ...
    upiData.forEach(item => {
      ...
      try {
        ...
        if (
          !COUNTRY_SUBDIVISION_CACHE?.countiesOrParishesCache?.[parsedCountrySubdivision] &&
          !COUNTRY_SUBDIVISION_CACHE.countySubdivisionsCache?.[parsedCountrySubdivision]
        ) {
