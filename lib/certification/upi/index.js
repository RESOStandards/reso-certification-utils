...
const runUpiTests = async ({
  ...
  if (!((resoCommonFormatJson && Object.values(resoCommonFormatJson)?.length) || (pathToResoCommonFormatJson && pathToResoCommonFormatJson?.length))) {
  ...
  try {
    const parsedJson =
      (resoCommonFormatJson && Object.values(resoCommonFormatJson)?.length ? resoCommonFormatJson : JSON.parse(await readFile(pathToResoCommonFormatJson))) ?? {};
