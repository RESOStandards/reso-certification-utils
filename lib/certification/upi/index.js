...
const runUpiTests = async ({ pathToResoCommonFormatJson, version = DEFAULT_UPI_VERSION } = {}) => {
  ...
  try {
    ...
    upiData.forEach(item => {
      ...
      try {
        ...
        invalidIds.push({ upi: UniversalParcelId, error: 'Parsing Error' });
      }
    });
  } finally {
    ...
    if (testsPassed) {
      ...
      console.error(`UPI tests failed! Error count: ${invalidIds.length}`);
