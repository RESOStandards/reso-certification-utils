 ...
 *
 */
const getStatesOrProvincesForCountry = async (countryCode = DEFAULT_COUNTRY_CODE) => {
  ...
  try {
    referenceData = await readFile(`./lib/certification/upi/country-subdivisions/${countryCode?.toLowerCase()}/states.txt`, 'utf-8');
  } catch {
  ...
  try {
    const cache = parseCsv(referenceData, FIELD_LIST, { separator: '|' }).reduce((acc, cur) => {
      const { stateFipsCode } = cur;

      const key = stateFipsCode;

      if (stateFipsCode && !acc[key]) {
        acc[stateFipsCode] = { key, ...cur };
      }
...
const getCountiesOrParishesForCountry = async (countryCode = DEFAULT_COUNTRY_CODE) => {
  ...
  try {
    referenceData = await readFile(`./lib/certification/upi/country-subdivisions/${countryCode?.toLowerCase()}/counties.txt`, 'utf-8');
  } catch {
    throw new Error(`Could not find reference data for country code '${countryCode}'`);
  }
...
const getCountySubdivisionsForCountry = async (countryCode = DEFAULT_COUNTRY_CODE) => {
  ...
  try {
    referenceData = await readFile(
      `./lib/certification/upi/country-subdivisions/${countryCode?.toLowerCase()}/county-subdivisions.txt`,
      'utf-8'
    );
  } catch {
    throw new Error(`Could not find reference data for country code '${countryCode}'`);
...
const buildCountrySubdivisionCaches = async (countryCode = 'US') =>
  Object.freeze({
    statesOrProvincesCache: await getStatesOrProvincesForCountry(countryCode),
