...
const { getFormattedResourceName, writeFile } = require('../../schema/utils');
...
const runRcfTests = async ({
  ...
  try {
    ...
    const { daReport, ...rest } = await generateReports([payload], true);
    ...
    const ddReportPath = resolve(join(rcfResultsPath, 'data-dictionary-report.json'));
