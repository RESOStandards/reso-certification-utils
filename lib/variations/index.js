          ...
          Object.keys(metadataReportMap?.[resourceName] ?? {}).forEach(fieldName => {
            ...
            if (!ignoreFieldMapping) {
              if (fieldSuggestions?.length) {
                ...
                //check lookupValues
                Object.values(lookupValues ?? {}).forEach(({ lookupValue, isStringEnumeration = false } = {}) => {
                  //if (!isStringEnumeration) return;
                ...
                Object.values(legacyODataValues ?? {}).forEach(({ legacyODataValue, isStringEnumeration = false }) => {
                  //if (isStringEnumeration) return;
