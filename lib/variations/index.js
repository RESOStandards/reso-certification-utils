...
const computeVariations = async ({
  ...
  try {
    ...
    if (fromCli) LOG_INFO(`Time Taken: ${calculateElapsedTimeString(startTime)}\n`);
    ...
    if (fromCli) {
      LOG_INFO('Metadata info:', getMetadataInfo(referenceMetadataStats));
      LOG_INFO(`Time taken: ${calculateElapsedTimeString(startTime, true)}`);
    ...
    if (fromCli) {
      LOG_INFO(`Time taken: ${calculateElapsedTimeString(startTime, true)}`);
      LOG_INFO('Metadata info:', getMetadataInfo(metadataReportStats));
            ...
            if (!ignoreFieldMapping) {
              if (fieldSuggestions?.length) {
                ...
                if (!isExpansion) {
                  ...
                  // TODO
                  // LOG_INFO(`TODO: Expansion found! Resource: '${resourceName}', Field: '${fieldName}'`);
                }
              } else {
    ...
    if (fromCli) {
      LOG_INFO(`Time Taken: ${calculateElapsedTimeString(startTime)}`);
    }
...
const findVariations = async ({
  ...
  LOG_INFO(`Using fuzziness of up to ${Math.round(fuzziness * 100)}% of word length!\n`);

  const START_TIME = new Date();

  try {
    const metadataReportJson = JSON.parse(await readFile(pathToMetadataReportJson, { encoding: 'utf8' }));
    LOG_INFO(`Using metadata report: ${pathToMetadataReportJson}`);
    ...
    if (Object.values(report?.variations ?? {}).some(variation => variation?.length)) {
      ...
      LOG_INFO(`Results saved to '${VARIATIONS_RESULTS_FILE}'`);
    }
...
const updateVariations = async ({
  ...
  try {
    ...
    if (fromCli) {
      LOG_INFO('Processing suggestions...');
    }
