...
const {
  ...
  RESO_SERVICES_URL,
  // Legacy Cert API authorizer (ApiKey -> provider token). Kept as a fallback.
  CERT_AUTH_API_BASE_URL,
  CERT_AUTH_API_USERNAME,
  CURRENT_PROVIDER_UOI,
  CERTIFICATION_API_KEY,
  // Current Cert API authorizer: OAuth2 client-credentials -> provider token. This is the
  // format the Certification site prefills into the downloadable .env. The Variations Service
  // itself is unchanged — it still takes the provider token as a Bearer.
  TOKEN_URI,
  CLIENT_ID,
  CLIENT_SECRET,
  CLIENT_SCOPE
 ...
 */
// The Variations Service needs a provider token, which either Cert API authorizer can mint
// (both are accepted server-side): the current OAuth2 client-credentials grant, or the legacy
// ApiKey call. RESO_SERVICES_URL is always required — it's the Variations Service base URL.
const hasOAuth2Credentials = () => !!(TOKEN_URI && CLIENT_ID && CLIENT_SECRET);
...
const checkRequiredCredentials = () => {
  ...
  if (!RESO_SERVICES_URL) {
    missingItems.push('RESO_SERVICES_URL');
  }

  // Accept either scheme. If neither is complete, report the current (OAuth2) vars, since that's
  // what the Certification site prefills — the ApiKey path is a backward-compatible fallback.
  if (!hasOAuth2Credentials() && !hasApiKeyCredentials()) {
    if (!TOKEN_URI) missingItems.push('TOKEN_URI');
    if (!CLIENT_ID) missingItems.push('CLIENT_ID');
    if (!CLIENT_SECRET) missingItems.push('CLIENT_SECRET');
  }
...
const findVariations = async ({
  ...
  try {
    ...
    LOG(`Using metadata report: ${relative(process.cwd(), pathToMetadataReportJson)}`);
    ...
    if (Object.values(report?.variations ?? {}).some(variation => variation?.length)) {
      const variationsOutputPath = join(outputPath ?? '', CERTIFICATION_FILES.VARIATIONS_REPORT);

      await writeFile(variationsOutputPath, Buffer.from(JSON.stringify(report, null, 2)));
      // Display the path relative to the working directory so it is meaningful both from the
      // CLI and in Docker, where the absolute container path is not the user's host path.
      // runDDTests passes an already-resolved absolute outputPath, so relativize against cwd
      // for the message; the write above uses the real path unchanged.
      LOG(`Results saved to '${relative(process.cwd(), variationsOutputPath)}'`);
    }
...
/**
 * OAuth2 client-credentials grant against the Cert API (current authorizer) -> provider token.
 * Mirrors the client-credentials flow in lib/replication/services/auth/oauth2.js: HTTP Basic
 * auth with the client id/secret, grant_type=client_credentials, optional scope. The returned
 * access_token IS the provider token the Variations Service expects as a Bearer.
 * @returns {Promise<{token?: string}>} the provider token, or an empty object on failure
 */
const fetchOAuth2ProviderToken = async () => {
  try {
    const params = new URLSearchParams();
    params.append('grant_type', 'client_credentials');
    if (CLIENT_SCOPE) {
      params.append('scope', CLIENT_SCOPE);
    }
    ...
    if (!response.ok) {
      console.error(`Could not fetch OAuth2 provider token from Cert API! ${response.status} ${response.statusText}`);
      return {};
    }
    ...
    console.error(`Could not fetch OAuth2 provider token from Cert API! TOKEN_URI: ${TOKEN_URI}`);
    return {};
  }
};

/**
 * Legacy Cert API authorizer: ApiKey -> provider token. Retained as a fallback for .env files
 * that predate the OAuth2 authorizer.
 * (CERT_AUTH_API_BASE_URL, CURRENT_PROVIDER_UOI, CERT_AUTH_API_USERNAME, CERTIFICATION_API_KEY)
 * @returns {Promise<{token?: string}>} response JSON containing the provider token,
 *   or an empty object on failure
 */
const fetchApiKeyProviderToken = async () => {
...
/**
 * Obtains a provider token via whichever Cert API authorizer the environment is configured for,
 * preferring the current OAuth2 client-credentials grant and falling back to the legacy ApiKey.
 * @returns {Promise<{token?: string}>} the provider token, or an empty object on failure
 */
const fetchProviderToken = async () => {
  if (hasOAuth2Credentials()) {
    return await fetchOAuth2ProviderToken();
  }
  return await fetchApiKeyProviderToken();
};
...
module.exports = {
  ...
  isValidUrl,
  hasValidSearchInput,
  // Exported for testing the dual-scheme Variations Service auth.
