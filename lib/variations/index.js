...
const { convertVariationsCsvToJson, convertVariationsJsonToCsv } = require('./utils');
...
const findVariations = async ({
  ...
  version = DEFAULT_DD_VERSION,
  useSuggestions = true,
  outputVariationsReportCsv = false,
  fromCli = false
} = {}) => {
  // custom logger
  ...
  try {
    ...
    if (Object.values(report?.variations ?? {}).some(variation => variation?.length)) {
      await writeFile(CERTIFICATION_FILES.VARIATIONS_REPORT, Buffer.from(JSON.stringify(report)));
      LOG(`Results saved to '${VARIATIONS_RESULTS_FILE}'`);
    }

    if (!!outputVariationsReportCsv && report?.variations) {
      await writeFile(CERTIFICATION_FILES.VARIATIONS_REPORT_CSV, Buffer.from(JSON.stringify(convertVariationsJsonToCsv(report))));
    }
