...
const { buildMetadataMap, getLogger, isValidUrl, CERTIFICATION_FILES, NOT_OK } = require('../../common');
...
const computeVariations = async ({
  ...
  try {
    ...
    if (fromCli) logger.info(`Time Taken: ${calculateElapsedTimeString(startTime)}\n`);
    ...
    if (fromCli) {
      logger.info('Metadata info:', getMetadataInfo(referenceMetadataStats));
      logger.info(`Time taken: ${calculateElapsedTimeString(startTime, true)}`);
    ...
    if (fromCli) {
      logger.info(`Time taken: ${calculateElapsedTimeString(startTime, true)}`);
      logger.info('Metadata info:', getMetadataInfo(metadataReportStats));
            ...
            if (!ignoreFieldMapping) {
              if (fieldSuggestions?.length) {
                ...
                if (!isExpansion) {
                  ...
                  // TODO
                  // logger.info(`TODO: Expansion found! Resource: '${resourceName}', Field: '${fieldName}'`);
                }
              } else {
    ...
    if (fromCli) {
      logger.info(`Time Taken: ${calculateElapsedTimeString(startTime)}`);
    }
...
const findVariations = async ({
  ...
  useSuggestions = true,
  fromCli = false,
  logger = getLogger(fromCli)
} = {}) => {
  logger.info('\n\nRunning Variations Check...');

  if (!pathToMetadataReportJson?.length) {
    logger.error(`Invalid value! pathToMetadataReportJson = '${pathToMetadataReportJson}'`);
    return;
  }

  if (fuzziness < 0 || fuzziness > 1) {
    logger.error('Invalid value! fuzziness must be a decimal number in the range [0, 1]');
    return;
  }

  logger.info(`Using fuzziness of up to ${Math.round(fuzziness * 100)}% of word length!\n`);

  const START_TIME = new Date();

  try {
    const metadataReportJson = JSON.parse(await readFile(pathToMetadataReportJson, { encoding: 'utf8' }));
    logger.info(`Using metadata report: ${pathToMetadataReportJson}`);
    ...
    if (useSuggestions && !checkRequiredCredentials()?.hasMissingItems) {
      ...
      logger.info('Skipping external suggestions...');
    }
    ...
    if (Object.values(report?.variations ?? {}).some(variation => variation?.length)) {
      ...
      logger.info(`Results saved to '${VARIATIONS_RESULTS_FILE}'`);
    }
