...
const VARIATIONS_SERVICE_URL = `${RESO_SERVICES_URL}/certification/variations`;
...
const fetchSuggestionsMap = async token => {
  try {
    const response = await fetch(VARIATIONS_SERVICE_URL, {
      headers: {
 ...
 * @returns
 */
const updateVariations = async ({ pathToCsvSuggestions }) => {
  try {
    ...
    if (response.ok) {
      const { updatedItemCount = 0 } = await response.json();
