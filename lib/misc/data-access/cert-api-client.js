...
/**
 * Gets Orgs data and turns it into a map.
 * @returns map containing org data
 * @throws exception if the orgs map cannot be processed
 */
...
/**
 * Retrieves system data from the USI reference sheet and creates a map from it.
 * @returns Map of system data keyed by providerUoi
 * @throws Error if the request fails or no data is found
 */
const getOrgSystemsMap = async () => {
  ...
  const { values: systems = [] } = await response.json();

  // JSON-based USI sheet data with header row
  if (systems?.length < 2) {
    throw new Error('No system data found!');
  }

  return (
    systems.slice(1).reduce((acc, [providerUoi, , usi]) => {
