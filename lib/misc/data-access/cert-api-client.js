...
/**
 * Fetches archived reports from the existing `/archived` endpoint with `getFullReport=true`.
 * The server-side handler injects `status` and `isLegacy` synthetic fields into each row;
 * we strip them so what's saved to disk matches the raw ES `_source` shape that restore
 * expects (plus the flattened `id`).
 *
 * Note: the existing endpoint is hard-capped at 1000 results with no pagination.
 *
 * @param {Object} args serverUrl
 * @returns array of archived reports (possibly empty)
 */
const fetchAllArchivedReports = async ({ serverUrl = '' } = {}) => {
  try {
    const response = await fetch(`${serverUrl}/api/v1/certification_reports/archived?getFullReport=true`, {
      method: 'POST',
      headers: {
        Authorization: `ApiKey ${CERTIFICATION_API_KEY}`,
        'Content-Type': 'application/json',
    ...
    // Strip synthetic fields injected by the existing endpoint's service.
    // `status` and `isLegacy` aren't part of the ES `_source` — they're computed
    // for the UI. Saving them would round-trip them into `_source` on restore.
    return rows.map(({ status: _status, isLegacy: _isLegacy, ...rest }) => rest);
  } catch (err) {
    console.log(chalk.redBright.bold(`Could not fetch archived reports: ${err.message}`));
    return [];
  }
};
