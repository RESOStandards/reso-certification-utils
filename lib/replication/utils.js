...
// custom libs
const { sleep, NOT_OK, resolveFilePathSync, getLogger } = require('../../common');
  ...
  records.forEach(record => {
    ...
    (
      ...
      if (!REPLICATION_STATE.getRecordCountHashMap()?.[normalizedRecordHash]) {
        ...
        REPLICATION_STATE.getRecordCountHashMap()[normalizedRecordHash]++;
      }
 ...
 */
const writeSchemaValidationErrorReport = async ({ outputPath, errorMap = {}, logger = getLogger() }) => {
  const {
    stats: { totalErrors = 0 }
  } = errorMap ?? {};
  if (totalErrors > 0) {
    ...
    logger.info(`Schema validation errors written to: ${resolvedPath}`);
  }
};
 ...
 * @param {Object} params
 */
const writeAnalyticsReports = async ({ outputPath, version, serviceRootUri, replicationStateService: REPLICATION_STATE_SERVICE, logger = getLogger() }) => {
  try {
    ...
    logger.info(`Response info written to ${resolvedPath}`);
    ...
    logger.info(`Availability results written to ${resolvedAvailabilityPath}`);
  } catch (err) {
    logger.error(err);
  }
};
 ...
 */
const getSystemRuntimeInfo = ({ startTime, version, resourceAvailabilityMap = {}, logger = getLogger() }) => {
  logger.info(
 ...
 */
const handleError = async ({ error = {}, rateLimitedWaitTimeMinutes = 60, logger }) => {
  ...
  if (errorType === ERROR_TYPES.HTTP) {
    ...
    if (statusCodeInt === ERROR_CODES.HTTP.RATE_LIMITED) {
      logger.info(`  -> ${new Date().toISOString()} - Waiting ${rateLimitedWaitTimeMinutes}m before making the next request...`);
    ...
    try {
      if (errorData) {
        ...
        logger.error(getErrorMessage(errorType, errorString));
      }
    } catch (err) {
      throw new Error(err);
    }
 ...
 */
const fetchTopLevelResourceCount = async ({ resourceName, serviceRootUri, filter, authService, logger }) => {
  ...
  try {
    const baseUrl = `${serviceRootUri}${serviceRootUri?.endsWith('/') ? '' : '/'}`,
      query = new URL(`${resourceName}?$top=1&$count=true${filter ? `&$filter=${filter}` : ''}`, baseUrl).toString();

    logger.info(`Fetching top-level resource count for '${resourceName}': '${query}'...`);
