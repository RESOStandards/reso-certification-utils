 ...
 * @param {Object} params
 */
const writeAnalyticsReports = async ({ version, serviceRootUri, replicationStateService: REPLICATION_STATE_SERVICE }) => {
  try {
...
const buildRequestUrlString = ({
  ...
  if (strategy === REPLICATION_STRATEGIES.TOP_AND_SKIP) {
    ...
    return new URL(
      `${baseUri}?$top=${$top}&$filter=${timestampField} gt ${timestamp}${preparedFilter?.length ? ` and ${preparedFilter} ` : ''}${
        $expand?.length ? `&$expand=${$expand}` : ''
    ...
    return new URL(
      `${baseUri}?$top=${$top}&$filter=${timestampField} lt ${timestamp}${preparedFilter?.length ? ` and ${preparedFilter} ` : ''}${
        $expand?.length ? `&$expand=${$expand}` : ''
...
const fetchTopLevelResourceCount = async ({ resourceName, serviceRootUri, filter, authService }) => {
  ...
  try {
    const query = new URL(`${serviceRootUri}/${resourceName}?$top=1&$count=true${filter ? `&$filter=${filter}` : ''}`).toString();
