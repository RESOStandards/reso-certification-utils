...
const scorePayload = ({
  ...
  records.forEach(record => {
    ...
    (normalizeRecords({ expansionInfo, record, resourceName, recordHashCountMap }) || []).forEach(
      ({ isExpansion = false, modelName, parentModelName, record: normalizedRecord = {} } = {}) => {
        // duplicate detection
        const normalizedRecordHash = hashJsonData({ modelName, parentModelName, normalizedRecord });
        if (!recordHashCountMap[normalizedRecordHash]) {
          recordHashCountMap[normalizedRecordHash] = 1;
          ...
          recordHashCountMap[normalizedRecordHash]++;
        }
 ...
 * @param {Object} params
 */
const writeDataAvailabilityReport = async ({ version, serviceRootUri, resourceAvailabilityMap = {}, responses = [] }) => {
  try {
      ...
      JSON.stringify(
        {
          ...
          generatedOn,
          serviceRootUri,
          responses: responses.map(({ requestUri, ...otherResponseInfo }) => {
            return {
      ...
      JSON.stringify(
        {
          ...
          version,
          generatedOn,
          ...consolidateResults({ resourceAvailabilityMap, responses })
        },
        null,
        '  '
      )
