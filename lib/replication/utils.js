    ...
    if (!!expansionInfo) {
      ...
      recordsToProcess.push(
        ...expansionInfo.reduce((acc, { fieldName, modelName }) => {
          ...
          // need to add parentRecord after removing the expansions
          acc.parentRecord = {
        ...
        Object.entries(record).forEach(([fieldName, value]) => {
          ...
          if (!availabilityMap?.fieldAvailabilityMap?.[fieldName]) {
            availabilityMap.fieldAvailabilityMap[fieldName] = {
              resourceName: false ? expandedResourceName : resourceName,
              fieldName,
              frequency: 0
            };
          }
...
/* 
const processResponses = (resourceAvailabilityMap = {}) => {
  const {
    resources: consolidatedResources = [],
    expansions: consolidatedExpansions = [],
    ...remainingConsolidatedResourceAvailabilityMap
  } = consolidateResults(resourceAvailabilityMap);

  // Separate responses from availability stats - two separate reports
  const {
    resources = [],
    expansions = [],
    responses = []
  } = [...consolidatedResources, ...consolidatedExpansions].reduce(
    (acc, { responses = [], resourceName, parentResourceName, expansionInfo, ...rest }) => {
      ...
      // add resource info to responses
      acc.responses.push(...responses.map(response => {
        return {
          resourceName,
          parentResourceName,
          expansionInfo,
