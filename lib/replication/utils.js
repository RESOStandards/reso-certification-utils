...
// custom libs
const { sleep, NOT_OK, resolveFilePathSync, isValidValue } = require('../../common');
...
const hashJsonData = jsonData => {
  try {
    const sha3Available = getHashes().includes('sha3-256');
    const hash = sha3Available ? createHash('sha3-256') : createHash('sha256');
    return hash.update(JSON.stringify(jsonData)).digest('hex');
  } catch (err) {
    console.log(err);
    return null;
  }
