...
const createRequestsFromMetadataReport = ({ serviceRootUri, metadataReportJson = {}, filter, top, orderby }) => {
  ...
  const { requests = [] } = fields.reduce(
    (acc, { resourceName, fieldName, isExpansion, typeName }) => {
      ...
      if (isExpansion && !acc.expansions.has(resourceName + fieldName)) {
        ...
        acc.requests.push({ resourceName, fieldName, isExpansion, typeName });
      }
...
const createRequestFromParameters = ({ serviceRootUri, resourceName, expansions = [], filter, top }) => {
  return {
    requestUri: createODataRequestUri({ serviceRootUri, resourceName, expansions, filter, top }),
    resourceName,
    expansions
  };
};
...
const createODataRequestUri = ({ serviceRootUri, resourceName, expansions = [], filter, top }) => {
  ...
  try {
    ...
    if (!!expansions && expansions?.length) {
      searchParams.push(`$expand=${expansions.join(',')}`);
    }
