 ...
 */
const prepareFilterExpression = (filter, options = {}) => {
  const { originatingSystemName, originatingSystemId } = options;

  if (!filter || !filter?.length) return filter;

  if (!originatingSystemId || !originatingSystemName) return filter;

  if (originatingSystemName && originatingSystemName?.length) {
    return `${filter} and OriginatingSystemName eq '${originatingSystemName}'`;
  }

  if (originatingSystemId && originatingSystemId?.length) {
    // Note: OriginatingSystemID rather than OriginatingSystemId
    return `${filter} and OriginatingSystemID eq '${originatingSystemId}'`;
  }
