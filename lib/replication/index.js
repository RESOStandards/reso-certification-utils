...
const { NOT_OK, getLogger } = require('../../common');
...
const replicate = async ({
  ...
  originatingSystemId,
  replicationStateService = createReplicationStateServiceInstance(),
  ...
  try {
    ...
    replicationStateService.setMetadataMap(metadataReport);
    ...
    for await (const request of requests) {
      ...
      try {
        ...
        if (!replicationStateService.checkIfTopLevelResourceCountExists(resourceName)) {
          replicationStateService.setTopLevelResourceCount(
            resourceName,
        ...
        for await (const {
          ...
          try {
            ...
            if (hasError) {
              ...
              await handleError({ error, rateLimitedWaitTimeMinutes, logger });
            } else if (hasResults) {
              if (jsonSchemaValidation) {
                ...
                if (hasValidationErrors) {
                  logger.error(`Schema validation errors found in the ${resourceName} payload!`);
                  if (strictMode) {
              ...
              if (shouldGenerateReports) {
                scorePayload({
                  ...
                  hasError,
                  replicationStateServiceInstance: replicationStateService
                });
              }

              if (shouldSaveResults) {
                replicationStateService.incrementResourcePageCount(resourceName);

                const resultsPath = buildOutputFilePath({ outputPath, isoTimestamp: startTimeIsoTimestamp, resourceName });
                await mkdir(resultsPath, { recursive: true });
                await writeFile(
                  resolve(normalize(join(resultsPath, `page-${replicationStateService.getResourcePageCount(resourceName)}.json`))),
            ...
            if (!!limit && totalRecordsFetched >= limit) {
              logger.info(`Reached specified record limit of ${limit}\n`);
              break;
            }
          } catch (err) {
            logger.error(err);
    ...
    if (shouldGenerateReports) {
      try {
        if (jsonSchemaValidation && schemaValidationResults?.stats?.totalErrors > 0) {
          await writeSchemaValidationErrorReport({ outputPath, errorMap: schemaValidationResults, logger });
        } else {
        ...
        logger.error(`Could not write report! ${err}`);
        process.exit(NOT_OK);
      }
    }
