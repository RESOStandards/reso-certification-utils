...
const { REPLICATION_STRATEGIES } = require('./utils');
const { replicationIterator } = require('./replication-iterator');
const REPLICATION_STATE = require('./services/replication-state');
...
const replicate = async ({
  ...
  try {
    ...
    // only init state service if generating reports
    ...
    REPLICATION_STATE.setMetadataMap(metadataReport);
      ...
      try {
        for await (const {
          ...
          ...otherIteratorInfo
            ...
            initialRequestUri,
            strategy,
            authInfo: { bearerToken, clientCredentials },
            limit,
          ...
          try {
            ...
            if (hasResults) {
              ...
              if (shouldSaveResults) {
                REPLICATION_STATE.incrementResourcePageCount(resourceName);

                const resultsPath = buildOutputFilePath({ outputPath, isoTimestamp: startTimeIsoTimestamp, resourceName });
                await mkdir(resultsPath, { recursive: true });
                await writeFile(join(resultsPath, `page-${REPLICATION_STATE.getResourcePageCount(resourceName)}.json`), JSON.stringify(responseJson));
              }
            }
    ...
    if (shouldGenerateReports) {
      try {
        await writeDataAvailabilityReport({ version: DEFAULT_DD_VERSION, serviceRootUri, replicationStateService: REPLICATION_STATE });
      } catch (err) {
    ...
    if (fromCli) {
      displayRuntimeInfo({ version: DEFAULT_DD_VERSION, startTime, resourceAvailabilityMap: REPLICATION_STATE.getResourceAvailabilityMap() });
    }
  } catch (err) {
    console.log(err);
  }
