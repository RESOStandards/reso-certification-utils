...
const restore = async (options = {}) => {
              ...
              for (const result of results) {
                ...
                try {
                  if (result.endsWith('.json')) {
                    ...
                    console.log(
                      chalk.green.bold(
                        `Restoring ${result.slice(0, result.lastIndexOf('.'))} - ID: ${id}...`
                      )
                    );
  ...
  if (restoreFromBackup) {
    console.log(
      chalk.bold(
        [
          `Data Dictionary restored: ${chalk.greenBright.bold(BACKUP_STATS[METADATA_REPORT_JSON].length)}`,
          `Data Availability restored: ${chalk.greenBright.bold(BACKUP_STATS[DATA_AVAILABILITY_REPORT_JSON].length)}`,
          `Web API Server Core restored: ${chalk.greenBright.bold(BACKUP_STATS[WEB_API_REPORT_JSON].length)}`,
