...
const filterIgnoredPaths = p => !(p && p.startsWith('.'));
...
const restore = async (options = {}) => {
  ...
  if (isLocalPath(pathToResults)) {
    ...
    const providerUoiAndUsiPaths = (await readDirectory(pathToResults)).filter(filterIgnoredPaths);
      ...
      if (!orgMap[providerUoi]) {
        ...
        if (!systems?.length) {
          ...
          if (!systems.find(system => system === providerUsi)) {
            ...
            const recipientUoiPaths = (await readDirectory(pathToRecipientResults)).filter(filterIgnoredPaths);
