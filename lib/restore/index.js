...
const { getOrgsMap, getOrgSystemsMap, processDataDictionaryResults, restoreBackedUpReport } = require('../misc/data-access/cert-api-client');
const { CERTIFICATION_FILES, isValidUrl } = require('../../common');
...
const restore = async (options = {}) => {
  ...
  const { pathToResults, url, restoreFromBackup = false, archived = false } = options;
  ...
  if (isLocalPath(pathToResults)) {
    const orgMap = restoreFromBackup ? {} : await fetchOrgData();
    const orgSystemMap = restoreFromBackup ? {} : await fetchSystemData();
        ...
        for await (const recipientUoi of recipientUoiPaths) {
          ...
          if (!orgMap[recipientUoi] && !restoreFromBackup) {
            ...
            if (!results?.length) {
              ...
              // When restoring from a server backup, POST each JSON file directly to the restore endpoint
              for (const result of results) {
                let id, pathToReport;
                try {
                  if (result.endsWith('.json')) {
                    pathToReport = resolve(join(currentResultsPath, result));
                    const report = JSON.parse(await readFile(pathToReport, 'utf8'));
                    ({ id } = report);
                    if (archived) report.archived = true;
                    console.log(
                      chalk.green.bold(
                        `Restoring ${archived ? 'archived ' : ''}${result.slice(0, result.lastIndexOf('.'))} - ID: ${id}...`
                      )
                    );
                    const success = await restoreBackedUpReport({ serverUrl: url, report });
                  ...
                  BACKUP_STATS.failed.push(id || pathToReport);
                }
              }
            } else {
  ...
  if (restoreFromBackup) {
    const archivedSuffix = archived ? ' (archived)' : '';
    console.log(
      chalk.bold(
        [
          `Data Dictionary restored${archivedSuffix}: ${chalk.greenBright.bold(BACKUP_STATS[METADATA_REPORT_JSON].length)}`,
          `Data Availability restored${archivedSuffix}: ${chalk.greenBright.bold(BACKUP_STATS[DATA_AVAILABILITY_REPORT_JSON].length)}`,
          `Web API Server Core restored${archivedSuffix}: ${chalk.greenBright.bold(BACKUP_STATS[WEB_API_REPORT_JSON].length)}`,
          `Restores failed: ${chalk.redBright.bold(BACKUP_STATS.failed.length)}`,
          BACKUP_STATS.failed.length
            ? `Failed Restore IDs: ${chalk.redBright.bold(BACKUP_STATS.failed.join(', '))}`
            : ''
        ]
          .filter(Boolean)
          .join('\n')
