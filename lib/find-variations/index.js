...
const computeVariations = async ({
  ...
  try {
    ...
    const getDDWikiUrlForResourceName = resourceName =>
      referenceMetadata?.resources?.find(item => item?.resourceName === resourceName)?.wikiPageURL ?? null;

    const getDDWikiUrlForFieldName = (resourceName, fieldName) => {
      if (!(resourceName && fieldName)) return null;

      const { ddWikiUrl = null } = standardMetadataMap?.[resourceName]?.[fieldName] ?? null;
      return ddWikiUrl;
    };
    ...
    const getDDWikiUrlForLookupValue = (resourceName, fieldName, lookupValue) => {
      ...
      const { ddWikiUrl = null } = standardMetadataMap?.[resourceName]?.[fieldName]?.lookupValues?.[lookupValue] || {};
      return ddWikiUrl;
    };
    ...
    const getDDWikiUrlForODataLookupValue = (resourceName, fieldName, oDataLookupValue) => {
      ...
      const { ddWikiUrl = null } = standardMetadataMap?.[resourceName]?.[fieldName]?.legacyODataValues?.[oDataLookupValue] || {};
      return ddWikiUrl;
    };
        ...
        Object.keys(metadataReportMap?.[resourceName] ?? {}).forEach(fieldName => {
          ...
          if (!isStandardField) {
            ...
            Object.values(lookupValues).forEach(({ lookupValue, legacyODataValue, isStringEnumeration }) => {
              ...
              if (isStringEnumeration && lookupValue && lookupValue?.length) {
                    ...
                    const remainingSuggestions =
                      localLookupValueSuggestions.flatMap(
                        ({ suggestedResourceName, suggestedFieldName, suggestedLookupValue, ...suggestion }) => {
                          if (!metadataReportMap?.[suggestedResourceName]?.[suggestedFieldName]?.[suggestedLookupValue]) {
                            return [
                              {
                                resourceName,
                                fieldName,
              ...
              } else if (legacyODataValue && legacyODataValue?.length) {
                ...
                Object.values(legacyODataValues).forEach(({ lookupValue, legacyODataValue }) => {
                  ...
                  if (!isStandardLegacyODataValue) {
                    const localODataLookupValueSuggestions = suggestionsMap?.[resourceName]?.[fieldName]?.[lookupValue]?.suggestions ?? [],
                      ...
                      const remainingSuggestions =
                        localODataLookupValueSuggestions.flatMap(
                          ({ suggestedResourceName, suggestedFieldName, suggestedLegacyODataValue, ...suggestion }) => {
                            if (!metadataReportMap?.[suggestedResourceName]?.[suggestedFieldName]?.[suggestedLegacyODataValue]) {
                              return [
                                {
                                  resourceName,
                                  fieldName,
                                  ...
                                  strategy: MATCHING_STRATEGIES.SUGGESTION,
                                  ddWikiUrl: getDDWikiUrlForLookupValue(
                                    suggestedResourceName,
                                    suggestedFieldName,
                                    suggestedLegacyODataValue
                                  ),
...
const findVariations = async ({
  ...
  try {
    ...
    if (useSuggestions && !!PROVIDER_TOKEN) {
      ...
      const { mappings = {} } = await fetchSuggestionsMap(PROVIDER_TOKEN);
