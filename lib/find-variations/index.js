              ...
              if (!isExpansion) {
                ...
                const isMinMatchingLength = fieldName?.length > MIN_MATCHING_LENGTH,
                  ...
                  isSubstringMatch =
                    isMinMatchingLength &&
                    !!(
                      normalizedStandardFieldName.includes(normalizedFieldName) || normalizedFieldName.includes(normalizedStandardFieldName)
                    );
...
const findVariations = async ({
  ...
  try {
    ...
    LOG(`Loading metadata report: ${pathToMetadataReportJson}`);
    ...
    LOG(`Saving results to ${VARIATIONS_RESULTS_FILE}...`);
    ...
    LOG(
      [
        ...
        `Total runtime: ${calculateElapsedTimeString(TOTAL_START_TIME)}`
      ].join('\n')
    );
  } catch (err) {
