...
const processFieldNames = ({ payload = {}, nesting = [], schema = {}, results = [], resourceName }) => {
  Object.keys(payload).forEach(f => {
    ...
    if (Array.isArray(value)) {
      const updatedNestedPayloads = [];
      value.forEach(v => {
        if (v instanceof Object) {
          const { payload: updatedNestedPayload } = processFieldNames({
            payload: v,
            nesting: [f, 'properties'],
            schema,
            results,
            resourceName
          });
          updatedNestedPayloads.push(updatedNestedPayload);
        } else {
          updatedNestedPayloads.push(v);
        }
      });
      payload = {
        ...payload,
        [f]: updatedNestedPayloads
      };
    }

    if (value?.['constructor'] === Object) {
      const { payload: updatedNestedPayload } = processFieldNames({ payload: value, nesting, schema, results, resourceName });
      if (updatedNestedPayload !== value) {
        payload = {
          ...payload,
          [f]: updatedNestedPayload
        };
      }
    }
...
const validate = ({
  ...
  if (payload.value) {
    ...
    payload.value?.forEach(v => {
      const { results, payload: modifiedPayload } = processFieldNames({
        payload: v,
      ...
      stringListErrors = results;
      modifiedPayloads.push(modifiedPayload);
    });
    payload = { ...payload, value: modifiedPayloads };
  } else {
    const { results, payload: modifiedPayload } = processFieldNames({
      payload,
      nesting: [resourceName, 'properties'],
  ...
  const isRCF = !!payload['@reso.context'];
...
const generateErrorReport = ({
  ...
  validate.errors.reduce(
    (acc, { instancePath, message, keyword, params, failedItemValue: value, isWarning, schemaPath, transformedValue }) => {
      ...
      let failedItemValue =
        value ??
        (keyword === SCHEMA_ERROR_KEYWORDS.ENUM
