...
const generateErrorReport = ({
  ...
  validate.errors.reduce(
    (acc, { instancePath, message, keyword, params, failedItemValue: value, isWarning, schemaPath, transformedValue }) => {
      ...
      if (!resourceNameFromSchemaPath && metadataMap?.[resourceName]?.[fieldName]?.isExpansion) {
        resourceNameFromSchemaPath = metadataMap[resourceName][fieldName].typeName;
      }
      ...
      if (resourceNameFromSchemaPath && resourceNameFromSchemaPath !== resource) {
        ...
        if (resourceNameFromSchemaPath !== failedItemName) {
          resourceName = resourceNameFromSchemaPath;
        }
      }
      ...
      //sanitize params
      [resourceName, failedItemName].forEach(checkMapParams);
      ...
      if (isWarning) {
        updateCacheAndStats({ cache: warningsCache, resourceName, failedItemName, message, fileName, stats, failedItemValue, isWarning });
        return acc;
      }

      updateCacheAndStats({ cache: errorCache, resourceName, failedItemName, message, fileName, stats, failedItemValue });
      return acc;
    },
    {}
  );
