...
const validate = ({ jsonSchema = {}, resourceName = 'Property', jsonPayload, errorMap = {}, fileName = '', ddVersion }) => {
  ...
  const oldOneOf = _.cloneDeep(schema.oneOf);
...
function validatePayload({ payloads = {}, schema, resourceNameFromArgs = '', versionFromArgs }) {
  /**
...
function combineErrors({ cache, errorCache, warningsCache, payloadErrors, stats }) {
  const errorReport = [];
