...
const validatePayload = ({ payloads = {}, schema, resourceNameFromArgs = '', versionFromArgs }) => {
  ...
  Object.entries(payloads).forEach(([fileName, payload]) => {
    ...
    errorMap = validate({
      ...
      fileName,
      errorMap,
      jsonSchema: schema instanceof Map ? schema.get(version) : schema,
...
const getResourceAndVersion = ({ payload, resource, version }) => {
  ...
  } else if (payload['@odata.context']) {
    const { resource: parsedResource, version: parsedVersion } = parseResoUrn(payload['@odata.context']);
    return {
      resource: parsedResource,
      version: parsedVersion
    };
