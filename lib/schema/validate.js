...
const validate = ({ jsonSchema = {}, resourceName = 'Property', jsonPayload, errorMap = {}, fileName = '', ddVersion }) => {
  ...
  if (payload.value) {
    ...
    results = processFieldNames({ payload, nesting: [resourceName, 'properties'], schema: schema.definitions });
  ...
  const stringListErrors = results
    .flatMap(r => {
  ...
  if (stringListErrors.length) {
    valid = false;
    stringListErrors.forEach(e => validate.errors.push(e));
  }
