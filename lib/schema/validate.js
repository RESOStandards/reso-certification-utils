...
const resolveStringList = (string, enums) => {
  // Quick check if the entire string is a valid enum, return it as an array
  if (enums.includes(string)) return [string];

  // Split the string on commas not followed by a space
  return string.split(/,(?!\s)/).reduce((acc, part) => {
    // Split each part on comma
    const subParts = part.split(',');

    // Check if any sub-part is in enums, if so, add all sub-parts to accumulator
    // Otherwise, add the original part to ensure spaces within are preserved
    if (subParts.some(subPart => enums.includes(subPart.trim()))) {
      acc.push(...subParts);
    } else {
      acc.push(part);
    }
  ...
  Object.keys(payload).forEach(f => {
    ...
    if (schemaTypeForField?.type === 'array' && schemaTypeForField?.items?.enum) {
      if (typeof value === 'string' && value?.includes(',')) {
        payload[normalizedField] = resolveStringList(value, schemaTypeForField.items.enum);
      }
    }
...
const validate = ({
  ...
  const stringListErrorsFormatted = stringListErrors
    .flatMap(r => {
      ...
      const parsedLookupValue = typeof value === 'string' ? resolveStringList(value, enums) : Array.isArray(value) ? value : [value];
