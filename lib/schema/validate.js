 ...
 */
const processFieldNames = (payload = {}) => {
  Object.keys(payload).forEach(f => {
    const value = payload[f];

    if(Array.isArray(value)){
      value.forEach(v => v instanceof Object && processFieldNames(v));
    }

    if(value instanceof Object){
      processFieldNames(value);
    }
...
// TODO: use constants from common
...
const validate = ({ jsonSchema = {}, resourceName = 'Property', jsonPayload, errorMap = {}, fileName = '', ddVersion }) => {
  ...
  if(payload.value){
    payload.value?.forEach(v => processFieldNames(v));
  }else{
    processFieldNames(payload);
  }
...
const ENUM_MISMATCH_ERROR_MESSAGE = 'must be equal to one of the allowed values';
...
const generateErrorReport = ({
  ...
  validate.errors.reduce((acc, { instancePath, message, keyword, params }) => {
    ...
    let failedItemValue =
      message === ENUM_MISMATCH_ERROR_MESSAGE
