...
const { generateRcfReports, generateDAReport } = require('./create-report');
 ...
 */
const generateReports = async (payloads, onlyDA) => {
  ...
  if (onlyDA) {
    return generateDAReport(normalizedPayloadsMap);
  }
  return generateRcfReports(normalizedPayloadsMap);
};
