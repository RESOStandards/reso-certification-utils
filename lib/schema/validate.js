...
const { DEFAULT_DD_VERSION, readZipFileContents } = require('../../common');
const {
...
/**
 * Accepts payload as an object or as a zip buffer
 * @param {Object} config
 * @param {string} config.resourceName
 * @param {string} config.version
 * @param {{}} config.jsonSchema
 * @param {Record<string, {}>|Buffer} config.jsonPayload
 * @param {{}} config.errorMap
 * @param {Object} config.validationConfig
 *
 * @returns Intermediate error and warning caches along with stats. Can be combined later using `combineErrors`
 */
const validate = async config => {
  let { errorMap = {} } = config;
  if (config.jsonPayload instanceof Buffer) {
    let payload;
    try {
      payload = await readZipFileContents(config.jsonPayload);
    } catch {
      const payloadErrors = config?.errorMap?.payloadErrors ?? {};
      addPayloadError(null, '', 'Invalid Zip file', payloadErrors, { totalErrors: 0, totalWarnings: 0 });
      return { ...errorMap, payloadErrors };
    }
    Object.entries(payload ?? {}).forEach(([fileName, payload]) => {
      errorMap = validateJSONPayload({
        ...config,
    ...
    return validateJSONPayload(config);
  }
};
