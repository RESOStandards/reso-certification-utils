 ...
 */
const processFieldNames = ({ payload = {}, nesting = [], schema = {} }) => {
  Object.keys(payload).forEach(f => {
    const value = payload[f];

    if (Array.isArray(value)) {
      value.forEach(v => v instanceof Object && processFieldNames({ payload: v, nesting: [f, 'properties'], schema }));
    }

    if (value?.['constructor'] === Object) {
      processFieldNames({ payload: value, nesting, schema });
    }
    ...
    const schemaTypeForField = [...nesting, normalizedField].reduce((acc, curr) => {
      if (acc?.[curr]) return acc[curr];
      return acc;
    }, schema);

    if (schemaTypeForField.type === 'array') {
      void 0;
    }
...
const validate = ({ jsonSchema = {}, resourceName = 'Property', jsonPayload, errorMap = {}, fileName = '', ddVersion }) => {
  ...
  // adjust fields containing '@'
  if (payload.value) {
    payload.value?.forEach(v => processFieldNames({ payload: v, nesting: [resourceName, 'properties'], schema: schema.definitions }));
  } else {
    processFieldNames({ payload, nesting: [resourceName, 'properties'], schema: schema.definitions });
...
const generateErrorReport = ({
  ...
  validate.errors.reduce((acc, { instancePath, message, keyword, params }) => {
    ...
    let failedItemValue =
      keyword !== 'additionalProperties' && !message?.includes('decimal') // decimal values tend to be a lot and make the report hard to read. So we just group them together.
