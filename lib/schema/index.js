...
const schema = async ({
  ...
  try {
    ...
    if (!isValidDataDictionaryVersion(version)) {
      throw new Error(`Invalid Data Dictionary version: '${version}'`);
    }
    ...
    if (validate) {
      if (!payloadPath) {
        throw new Error('Invalid path to payloads');
      }

      if (!(resourceName && version)) {
        throw new Error('Resource name (-r, --resourceName) and version (-v, --version) should be passed together');
      }
