...
const { validatePayload, isValidDdVersion, validate, combineErrors, VALIDATION_ERROR_MESSAGES } = require('./validate');
  ...
  try {
    ...
    if (result?.errors) {
      ...
      if (!success) {
        ...
        console.log(chalk.yellowBright.bold(`Written the validation errors to the file ${errorPath}/${ERROR_REPORT}`));
      }
    } else {
