...
const changeKeyMapToList = errorCache => {
  Object.values(errorCache).forEach(v => {
    Object.values(v.resources).forEach(re => {
      const keys = Object.keys(re.keys ?? {});
      if (keys.length) {
        re.keys = keys;
      }
    });
...
const updateCacheAndStats = ({
  ...
  if (failedEnum !== null && failedEnum !== undefined) {
    ...
    if (sourceModelField && !sourceModelField) {
