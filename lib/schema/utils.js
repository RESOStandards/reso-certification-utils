...
const { getStandardMetadata } = require('@reso/reso-certification-etl/lib/common');
 ...
 */
const combineErrors = ({ errorCache, warningsCache, stats = {
  totalWarnings: 0,
  totalErrors: 0
}, version }) => {
  return {
...
const getFormattedResourceName = (resource, version) => {
  const metadata = getStandardMetadata(version);
  const resources = metadata.fields.reduce((acc, curr) => {
    if (!acc[curr.resourceName]) {
      acc[curr.resourceName] = true;
    }
    return acc;
  }, {});
  return Object.keys(resources).find(r => r.toLowerCase() === resource?.toLowerCase());
};
...
const isValidIsoDateTimeOffset = value => {
  if (typeof value !== 'string') return false;
  const match =
  ...
  const tzZulu = match[8] === 'Z';
  // eslint-disable-next-line prefer-destructuring
  const tzSign = match[9];
  const tzHour = match[10] == null ? 0 : Number(match[10]);
  const tzMinute = match[11] == null ? 0 : Number(match[11]);

  if ([year, month, day, hour, minute, second, tzHour, tzMinute].some(n => !Number.isFinite(n))) return false;
  if (month < 1 || month > 12) return false;
  if (day < 1 || day > 31) return false;
