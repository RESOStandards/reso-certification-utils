...
function areAllEnumsStringType(enums) {
  return enums.every(item => {
    if (item.includes(' ')) {
      return true;
    }
    const hasCamelCase = /[a-z][A-Z]/.test(item);
...
const generateDDReport = ({ daReport, metadataMap, payloadCache, expansionFields }) => {
  ...
  const stringOrLegacyOdataClassificationMap = Object.entries(payloadCache).reduce((acc, [resourceName, fieldMap]) => {
    Object.entries(fieldMap).forEach(([fieldName, values]) => {
      const isEnumType = values.length > 0 && values.every(v => typeof v === 'string');
      const isStringType = isEnumType && areAllEnumsStringType(values.filter(v => typeof v === 'string'));
      if (isStringType) {
